// client/src/pages/admin/AdminRepuestos.jsx
// RF10 FIX: POST /api/repuestos y DELETE /api/repuestos/:id ya existen en el backend
// RF10 FIX: agrega campo imagen_url al formulario
...
const crearRepuesto   = (data) => apiFetch("/repuestos",      { method: "POST",   body: JSON.stringify(data) });
const eliminarRepuesto = (id)  => apiFetch(`/repuestos/${id}`, { method: "DELETE" });
const actualizarStock  = (id, stock) => apiFetch(`/repuestos/${id}`, { method: "PUT", body: JSON.stringify({ stock }) });

const FORM_VACIO = {
  nombre: "", descripcion: "", categoria: "",
  marca: "", modelo_compatible: "", stock: 0, precio: 0, imagen_url: "",
};
...
export default function AdminRepuestos() {
  ...
  const abrirCrear = () => {
    setModoEdit(false); setForm(FORM_VACIO); setFormId(null); setErrForm(""); setShowForm(true);
  };
  ...
  const abrirEditar = (r) => {
    ...
    setForm({
      ...
      stock:             r.stock ?? 0,
      precio:            r.precio ?? 0,
      imagen_url:        r.imagen_url || "",  // RF10 FIX
    });
    setErrForm(""); setShowForm(true);
  };
  ...
  const handleGuardar = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        imagen_url:        form.imagen_url.trim() || null,  // RF10 FIX
      };
      if (modoEdit) {
    ...
    <div style={s.page}>
      ...
      {stockSummary.critico > 0 && (
        <div style={{ background: "#fff7ed", border: "1px solid #fed7aa", borderRadius: 12, padding: "12px 18px", marginBottom: 16, display: "flex", alignItems: "center", gap: 10, fontSize: 14, fontWeight: 600, color: "#92400e" }}>
          ...
          <button style={{ marginLeft: "auto", background: "#f97316", color: "#fff", border: "none", padding: "4px 12px", borderRadius: 7, cursor: "pointer", fontSize: 12, fontWeight: 700 }} onClick={() => setFiltStock("critico")}>
            Filtrar
          </button>
        </div>
      )}
      ...
      <div style={{ display: "flex", gap: 12, marginBottom: 20, flexWrap: "wrap" }}>
        {[
          ...
          <div key={item.label} onClick={() => setFiltStock(filtStock === item.id ? "" : item.id)}
            style={{ background: item.bg, border: `2px solid ${filtStock === item.id ? item.color : item.border}`, borderRadius: 10, padding: "10px 18px", cursor: "pointer", display: "flex", alignItems: "center", gap: 10 }}>
      ...
      <div style={s.filtrosBox}>
        <input placeholder="Buscar por nombre, categoría, marca o ID..." value={busqueda} onChange={e => setBusqueda(e.target.value)} style={{ ...s.input, minWidth: 240 }} />
        <select value={filtCat} onChange={e => setFiltCat(e.target.value)} style={s.select}>
          <option value="">Todas las categorías</option>
          {categorias.map(c => <option key={c} value={c}>{c}</option>)}
        </select>
        <button style={s.btnClear} onClick={() => { setBusqueda(""); setFiltCat(""); setFiltStock(""); }}>Limpiar</button>
      </div>
      ...
      {!loading && (
        repFiltrados.length === 0 ? (
          <div style={{ textAlign: "center", padding: 60, color: "#94a3b8" }}>Sin repuestos que coincidan con los filtros</div>
        ) : (
          <div style={s.grid}>
            {repFiltrados.map(r => {
              ...
              return (
                <div key={r.id_repuesto} style={s.card}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
                    ...
                    <span style={{ background: stockBg, color: stockColor, border: `1px solid ${stockBorder}`, padding: "3px 10px", borderRadius: 20, fontSize: 12, fontWeight: 700 }}>
                      {r.stock <= 5 ? "⚠️ " : ""}{r.stock} uds
                    </span>
                  </div>

                  {/* RF10: mostrar imagen si existe */}
                  <div style={{ height: 80, textAlign: "center", marginBottom: 8 }}>
                    {r.imagen_url
                      ? <img src={r.imagen_url} alt={r.nombre} style={{ height: "100%", objectFit: "contain", borderRadius: 8 }} onError={e => { e.target.style.display = "none"; }} />
                      : <div style={{ fontSize: 30 }}>🔩</div>
                    }
                  </div>

                  <h4 style={{ margin: "0 0 6px", fontSize: 15, fontWeight: 800, color: "#1e293b", textAlign: "center", lineHeight: 1.3 }}>{r.nombre}</h4>

                  <div style={{ display: "flex", flexDirection: "column", gap: 4, marginBottom: 14 }}>
                    {r.categoria && <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12 }}><span style={{ color: "#94a3b8" }}>Categoría</span><span style={{ color: "#1e293b", fontWeight: 600 }}>{r.categoria}</span></div>}
                    {r.marca     && <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12 }}><span style={{ color: "#94a3b8" }}>Marca</span><span style={{ color: "#1e293b", fontWeight: 600 }}>{r.marca}</span></div>}
                    <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13 }}>
                      <span style={{ color: "#94a3b8" }}>Precio</span>
                      <span style={{ color: ACENTO, fontWeight: 800, fontSize: 15 }}>${Number(r.precio || 0).toLocaleString("es-CO")}</span>
                    </div>
                  </div>

                  {r.descripcion && <p style={{ fontSize: 12, color: "#64748b", marginBottom: 12, lineHeight: 1.5 }}>{r.descripcion}</p>}
        ...
        <div style={s.overlay} onClick={() => setShowForm(false)}>
          <div style={s.modal} onClick={e => e.stopPropagation()}>
            <div style={s.modalHeader}>
              <h3 style={{ margin: 0, color: ACENTO }}>{modoEdit ? `Editar repuesto #${formId}` : "Nuevo repuesto"}</h3>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              ...
              <div>
                ...
                <textarea value={form.descripcion} onChange={e => setForm(f => ({ ...f, descripcion: e.target.value }))} rows={3} style={{ ...s.inp, resize: "vertical" }} placeholder="Descripción breve..." />
              </div>
              {/* RF10 FIX: campo imagen_url */}
              <div>
                <label style={s.label}>URL de imagen (opcional)</label>
                <input value={form.imagen_url} onChange={e => setForm(f => ({ ...f, imagen_url: e.target.value }))} style={s.inp} placeholder="https://ejemplo.com/imagen.jpg" />
              </div>

              {errForm && (
                <div style={{ background: "#fef2f2", color: "#ef4444", padding: "10px 14px", borderRadius: 8, fontSize: 13, border: "1px solid #fca5a5" }}>{errForm}</div>
              )}
      ...
      {stockModal && (
        <div style={s.overlay} onClick={() => setStockModal(null)}>
          <div style={{ ...s.modal, maxWidth: 360 }} onClick={e => e.stopPropagation()}>
            ...
            <p style={{ color: "#64748b", fontSize: 14, marginBottom: 4 }}><strong>{stockModal.nombre}</strong></p>
            <p style={{ color: "#94a3b8", fontSize: 13, marginBottom: 16 }}>Stock actual: <strong>{stockModal.stock}</strong></p>
            <label style={s.label}>Nuevo stock</label>
            <input type="number" min={0} value={stockVal} onChange={e => setStockVal(e.target.value)} style={{ ...s.inp, marginBottom: 18 }} />
            <div style={{ display: "flex", gap: 10 }}>
...
const s = {
  ...
  card:       { background: "#fff", borderRadius: 16, padding: "20px 18px", boxShadow: "0 2px 10px rgba(0,0,0,0.07)", border: "1px solid #e2e8f0" },
